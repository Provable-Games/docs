...
Every player-facing action **must** be wrapped with `self.minigame.pre_action(token_id)` and `self.minigame.post_action(token_id)`:

- **`pre_action`** validates the token is playable (exists, not expired, game not over)
- **`post_action`** syncs score and game_over state back to the token contract

:::warning
Without these hooks, the token contract won't know when scores change or games end, and players could take actions on expired or completed tokens.
:::
...
4. **Required**: Call `pre_action`/`post_action` in every player-facing action
