...
Platforms call `mint()` on your game contract to create a game instance (an NFT) for each player. They then call `score()` to validate results for leaderboards and prizes. See: [Embedding Games](/embeddable-game-standard/implementation/embedding-games).
...
Access a library of onchain games without building from scratch. Add new content and engagement through variety. Each game instance is a unique, verifiable NFT.
