...
- `player_name`: Optional player name (`felt252`). Pass `Option::None` to omit.
...
- `tournament_distribution_shares`: Returns the basis-point shares array for tournaments configured with `Distribution::Custom`. `tournament()` returns `Custom([])` to keep that view small; callers that need the shares fetch them here. Returns an empty array for non-`Custom` distributions.
