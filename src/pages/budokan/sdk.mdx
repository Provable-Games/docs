...
// shares: number[] — basis points, summing to 10000
```

> **Player queries:** the client doesn't expose direct `getPlayer*` methods. To fetch a player's tournaments or earnings, use the [React hooks](#data-hooks) `useTournamentsByOwner` and `usePlayerRewards`, which compose the underlying token-ownership data.
