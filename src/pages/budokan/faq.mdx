...
Budokan supports the Cartridge Controller (recommended) along with the broader set of Starknet wallets. See the [Onboarding Guide](/budokan/guide/onboarding) for the current list and setup instructions.
...
Budokan supports any ERC20 token listed on the Ekubo API, plus a curated set of ERC721 (NFT) collections for prizes. ERC721 entry fees are not yet supported. See [Entry Fees](/budokan/guide/create/entry-fees) and [Prizes](./guide/prizes).
