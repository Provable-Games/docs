...
After a tournament ends, there is a dedicated window—called the **submission period**—for players to submit their scores on-chain. The tournament creator sets this window (minimum 1 hour). It is crucial to submit your score during this time; otherwise, you may not qualify for prizes or leaderboard positions.
...
Provable Games runs a global submission bot that automatically detects when tournaments end and submits the top scores on-chain. This means most players don't need to worry about manually submitting — scores are handled automatically.

The submission bot is also open source, so anyone can run their own instance. You can find the code and instructions here:
